...
(async () => {
  ...
  await fs.writeFile("charlson-comorbidities-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("charlson-comorbidities-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
